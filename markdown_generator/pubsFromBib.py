...
links_order = ["Paper","Arxiv","Project","Supplementary","Video","Code"]
def html_escape(text):
    """Produce entities within text."""
    return "".join(html_escape_table.get(c,c) for c in text)
def collect_authors(bibdata, bib_id, markdown_item):
    # Build Citation from text
    citation = ""
    authors_list = []  # Use a list to store authors
    ...
    for i, author in enumerate(bibdata.entries[bib_id].persons["author"]):
        ...
        if i == 0:
            markdown_item['selected'] = 'true' if author_name == my_name else "false"
        
        authors_list.append(author_name)
    
    markdown_item['authors'] = authors_list  # Store the list of authors
    
    # Format authors as a simple list
    # authors_output = ""
    # for author in markdown_item['authors']:
    #     authors_output += f"- {author}\n"
    
    # markdown_item['authors'] = authors_output
...
def collect_from_folder(file_path):
    ...
    for item in sub_items:
        if "teaser" in item:
            markdown_item['cover'] = os.path.join("/",file_path,item)
        if "main.pdf" == item:
            links["Paper"] = os.path.join("/",file_path,item)
        ...
        if "content.md" == item:
            with open(os.path.join(file_path,item), 'r', encoding='utf-8') as f:
                content = f.read()
    markdown_item['layout'] = "publication"
    markdown_item['links'] = links
    ...
    for bib_id in bibdata.entries:
        ...
        try:
            ...
            markdown_item['pub_date'] = pub_year
            ...
            for i,author in enumerate(bibdata.entries[bib_id].persons["author"]):
                ...
                if i==0:
                    markdown_item['selected'] = 'true' if author == my_name else "false"
                if i!=0:
                    authors = authors + ", "
                if author == my_name:
                    author == f"**{author}**"
            ...
            markdown_item = collect_authors(bibdata,bib_id,markdown_item)
            #citation title
            ...
            for key in markdown_item.keys():
                if key == "authors":  # Special handling for authors to format as a Markdown list
                    md += "\nauthors:"
                    for author in markdown_item[key]:  # Assuming markdown_item['authors'] is a list
                        md += f"\n- {author}"
                elif key == "links":
                    md += "\nlinks:"
                    # 按照links order 排序，其他的按照本来的顺序放在后面
                    links_name = list(markdown_item[key].keys())
                    links_name_sorted = []
                    for link in links_order:
                        if link in links_name:
                            links_name_sorted.append(link)
                            links_name.remove(link)
                    links_name_sorted = links_name_sorted + links_name
                    for link_name in links_name_sorted:
                        link_url = markdown_item[key][link_name]
                        md += f"\n  {link_name}: {link_url}"
                    # for link_name, link_url in markdown_item[key].items():
                    #     md += f"\n  {link_name}: {link_url}"
                else:
        ...
        # field may not exist for a reference
            ...
            continue
root_dir = "assets/files/publications"
